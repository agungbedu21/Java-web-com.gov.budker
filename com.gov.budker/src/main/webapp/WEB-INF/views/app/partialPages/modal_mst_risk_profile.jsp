...
<div data-backdrop="static" class="modal fade in" id="modalRiskProfile" role="dialog" aria-labelledby="myModalLabel"
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 style="font-weight: 500;color: #000;" class="modal-title" id="myModalLabel">
                    <c:if test="${dataList.riskProfileId == null}">Tambah </c:if>
                    <c:if test="${dataList.riskProfileId != null}">Ubah </c:if>
                    Profile Resiko
                    <c:if test="${dataList.riskProfileId == null}"> Baru</c:if>
                </h3>
            </div>
            <div class="modal-body">
                <input type="hidden" id="riskProfileId" name="riskProfileId" value="${dataList.riskProfileId}">
                <div class="row">
                    <div class="col-md-12">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <label><b>Nama Profile Resiko</b></label>
                            <input name="riskProfileName" id="riskProfileName" type="text" class="form-control"
                                   placeholder=""
                                   value="${dataList.riskProfileName}">
                        </div>
                    </div>
                </div>
                <div class="row">
...
<script type="text/javascript">
    ...
    function validate() {
        $('#modalRiskProfile').bootstrapValidator('validate');
    }

    function submit() {
        var data = {
            "riskProfileId": $('#riskProfileId').val(),
            "year": $('#year').val(),
            "riskProfileName": $('#riskProfileName').val(),
            "carMinValue": $('#carMinValue').val(),
            "status": $('#status').val()
        };
        var idData = $("#riskProfileId").val();
        var is = null;
        if (idData === "") {
            is = 1;
        } else {
        ...
        $.ajax({
            ...
            'type': 'POST',
            'url': APP_PATH + '/cal/riskprofile/saveOrUpdate/' + is,
